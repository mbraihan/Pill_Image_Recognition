...
def csv_from_xml(directory, path_name=""):
    ...
    for image in image_paths:
        target_filename = os.path.join(
            path_name, image) if path_name else image
...
def crop_and_save(
    ...
    image_df["image_path"] = ChangeToOtherMachine(
    ...
    for index, row in image_df.iterrows():
        ...
        cropped = imageObject.crop(
            (row["xmin"], row["ymin"], row["xmax"], row["ymax"]))
        label = row["label"]
        if type(label) == int:
            label = label_dict[label]
        image_name_cropped = (
            "_".join([row["image"][:-4], postfix,
                      label, str(counter)]) + ".jpg"
        )
...
if __name__ == "__main__":
    ...
    multi_df = pd.read_csv(
        "C:/Users/Fahim/Documents/Insight/eq/EQ_new/Train_Housing_detector/2/vott-csv-export/Housing_cropping-export.csv"
    )
    ...
    path = "C:/Users/Fahim/Documents/Insight/eq/EQ_new/Train_Window_Detector/base"
    csv_from_xml(path, "/home/ubuntu/logohunter/data/windows").to_csv(
        "C:/Users/Fahim/Documents/Insight/eq/EQ_new/Train_Window_Detector/base/annotations.csv"
    )
